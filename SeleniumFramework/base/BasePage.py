...
class BaseClass:
    ...
    def getWebElement(self, locatorValue, locatorType="id"):
        ...
        try:
            ...
            #assert False  # Throw exception test fails
        return webElement
    ...
    def DragNDrop(self, locatorValue1, locatorValue2, locatorType="id"):
        actions = ActionChains(self.driver)
        try:
            locatorType = locatorType.lower()
            drag = self.waitForElement(locatorValue1, locatorType)
            drop = self.waitForElement(locatorValue2, locatorType)
            # actions.move_to_element(webElement).perform()

            actions.drag_and_drop(drag, drop).perform()  # drag and drop opration

            self.log.info(
                "drag to WebElement with locator value " + locatorValue1 + " using locatorType " + locatorType)
        except:
            self.log.error(
                "Unable to drag to WebElement with locator value " + locatorValue1 + "using locatorType " + locatorType)
            print_stack()
            self.takeScreenshot(locatorType)

    def radioButtons(self, locatorValue, locatorType="name"):
        webElement = None
        try:
            locatorType = locatorType.lower()
            locatorByType = self.getLocatorType(locatorType)
    ...
    def checkBoxes(self, locatorValue, locatorType="name"):
        webElement = None
        try:
            locatorType = locatorType.lower()
            locatorByType = self.getLocatorType(locatorType)
    ...
    def dropDown(self, locatorValue, locatorType="id"):
        webElement = None
        try:
            locatorType = locatorType.lower()
            webElement = self.waitForElement(locatorValue, locatorType)
            self.log.info(
                "Dropdown WebElement with locator value " + locatorValue + " using locatorType " + locatorType)
        except:
            self.log.error(
                "Unable to dropdown to WebElement with locator value " + locatorValue + "using locatorType " + locatorType)
            print_stack()
            self.takeScreenshot(locatorType)

        return webElement

    def multiSelect(self, locatorValue, locatorType="id"):
        webElement = None
        try:
            locatorType = locatorType.lower()
            webElement = self.waitForElement(locatorValue, locatorType)
            self.log.info(
                "MultiSelect WebElement with locator value " + locatorValue + " using locatorType " + locatorType)
        except:
            self.log.error(
                "Unable to MultiSelect WebElement with locator value " + locatorValue + " using locatorType " + locatorType)
            print_stack()
            self.takeScreenshot(locatorType)
